...
//        $data = DB::table('approval_form_business as afb')
    ...
    public function projectList(Request $request, $model = null)
    {
        ...
        $res = $data->join('projects as ps', function ($join) {
            ...
            ->orderBy('cs.created_at', 'desc')
            ->select('afb.form_instance_number', 'cs.contract_number', 'cs.title', 'af.name as form_name', 'cs.creator_name', DB::raw("DATE_FORMAT(cs.created_at,'%Y-%m-%d %h:%i') as created_at"), 'afb.form_status', 'cs.stars', 'cs.star_type', 'cs.contract_money', 'cs.type', 'users.icon_url')->get()->toArray();
