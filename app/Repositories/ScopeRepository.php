...
//        dd($arrRoleId);
        ...
        $dataViewSql = RoleDataView::select('data_view_sql')->where('resource_id',$resourceId)->where('data_view_id',$dataDictionarieId)->get()->toArray();
...
//        $roleList = RoleUser::where('user_id',$userId)->select('role_id')->get();
    ...
    {
        //1.获取接口在数据字典中的id
        $resource= DataDictionarie::where('val', '/'.$uri)->where('code', $method)->select('id','parent_id','name')->first();//检查数据字典里是否配置了该权限，没有则放过该请求
